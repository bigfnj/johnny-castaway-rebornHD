"""Prepare the selected connecting baseline and eight explicit native clips."""
import argparse
import ast
import json
...
def main():
    ...
    assert sha(archive.read_bytes()) == config.BASELINE_SHA, 'selected connecting baseline ZIP identity'
    assert sha((ROOT / 'assets/scrantic_data.zip').read_bytes()) == config.PRODUCTION_SHA, 'production ZIP unchanged'
    with zipfile.ZipFile(archive) as packed:
        ...
        assert len(names) == 2594 and len(accepted) == 43, 'selected connecting baseline coverage'
        assert all(f'data/styles/cartoon/BMP/JOHNWALK.BMP/{i:03}.png' in names for i in config.TARGET_FRAMES), 'all28 Johnny inputs present'
        index_path = ROOT / 'art/cartoon/skin-tone-v1/input-index.json'
        index = config.load_json(index_path)
        assert index['candidate_archive_sha256'] == config.BASELINE_SHA, 'frozen input baseline binding'
        for row in index['frames']:
            assert sha(packed.read(row['member'])) == row['sha256'], 'frozen input member:' + str(row['frame'])
...
'''
    ...
    record = {'source_commit': subprocess.check_output(['git', '-C', str(ROOT), 'rev-parse', 'HEAD'], text=True).strip(),
              ...
              'scope': 'Exact selected connecting baseline; six routes plus015/016 same-heading waits; no corrected candidate or production changes.',
              'input_index_sha256': sha(index_path.read_bytes()), 'selected_connecting_archive_sha256': config.BASELINE_SHA}
    (OUT / 'preparation.json').write_text(json.dumps(record, indent=2) + '\n', encoding='utf-8')
    print('PASS prepared exact connecting baseline, eight-clip observer and independent C trace helper')

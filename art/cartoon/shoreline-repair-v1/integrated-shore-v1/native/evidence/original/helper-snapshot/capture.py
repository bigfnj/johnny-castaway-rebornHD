...
def package_pair(baseline,candidate):
    ...
    with zipfile.ZipFile(candidate) as z:
        ...
        center=sizes['6'];require(center in ([320,50],[352,80]) and all(sizes[str(f)]==center for f in (7,8)),'uniform registered center foam canvases')
...
def verify_log(text,args,candidate,canvases):
    ...
    for actual,row in zip(surfaces,wave_rows):
        ...
        offset=-16 if candidate and 6<=f<=8 and [w,h]==[352,80] else 0
        require(actual==[f,(x+dx)*2+offset,(y+dy)*2,w,h],'actual high-wave surface placement')
    displays=[]
...
def scoped_pixels(before,after,offset):
    require(len(before)==len(after)==1280*960*3,'native pixel dimensions')
    # Union of actually drawn changed ground and high-wave surfaces. Expanded
    # center foam ends at718, inside this exact registered ground180px box.
    l,t,r,b=540+2*offset[0],548+2*offset[1],1180+2*offset[0],728+2*offset[1]
    changed=0
    for y in range(960):
        start=y*3840
        if not t<=y<b: require(before[start:start+3840]==after[start:start+3840],'pixel outside registered ground/foam');continue
...
def controls(output,case,canvases):
    folder=output/'day/none';text=(folder/'candidate/smoke/capture.log').read_text()
    ...
    wave_x=712 if canvases['6']==[352,80] else 728
    tests=[('actual_ground_offset',lambda:verify_log(text.replace('GROUND SURFACE: x=540 ','GROUND SURFACE: x=541 '),case,True,canvases),'actual ground surface placement'),
      ...
      ('outside_registered_pixel',lambda:scoped_pixels(before,bytes(damaged),[0,0]),'pixel outside registered ground/foam')]
    motion=output/'motion/high_none/candidate/smoke/capture.log'
    if motion.is_file():
        full=motion.read_text();verify_log(full,MOTION['high_none'],True,canvases)
        altered=re.sub(r'(phases=\d+,)8(,)',r'\g<1>7\2',full)
        tests.append(('full_phase_coverage',lambda:verify_log(altered,MOTION['high_none'],True,canvases),'all displayed native wave phases'))
    results=[]
...
def main():
    ...
    try:
        ...
        pair=package_pair(a.baseline,a.candidate);save(a.output/'inputs.json',{'pair':pair,'protected_sha256':protected,'helpers_sha256':{q.name:sha(q.read_bytes()) for q in HERE.iterdir() if q.suffix in ('.py','.c')},'scope':'Current port renderer, source unchanged during run. Explicit state selection bypasses calendar/cargo policy. Native timing is the port logical20ms tick, not original-executable proof.'})
        ...
        for group,cases in groups.items():
            for name,args in cases.items():
                ...
                for left,right in zip(rows['baseline']['displays'],rows['candidate']['displays']):
                    ...
                    counts.append(scoped_pixels(b,c,args[2:4]))
        ...
        controls(a.output,DAY['none'],pair['candidate_canvases'])

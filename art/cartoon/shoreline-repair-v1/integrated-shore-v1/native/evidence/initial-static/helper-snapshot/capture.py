...
def package_pair(baseline,candidate):
    ...
    with zipfile.ZipFile(candidate) as z:
        ...
        center=sizes['6'];require(center in ([320,50],[320,80]) and all(sizes[str(f)]==center for f in (7,8)),'uniform registered center foam canvases')
...
def verify_log(text,args,candidate,canvases):
    ...
    return {'displays':displays,'native_calls':calls,'native_returns':returns,'background_draws':bg,
      'ground_surface_placement':actual,'holiday_draws':props,'displayed_phases':sorted(seen),
...
def controls(output,case,report,canvases):
...
def main():
    ...
    try:
        ...
        controls(a.output,DAY['none'],results['day/none'],pair['candidate_canvases'])

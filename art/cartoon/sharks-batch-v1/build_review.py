"""Build the 39-drawing shark appearance gallery, with originals kept visible."""
import hashlib
import html
import json
...
ROOT = HERE.parents[2]
GROUPS = [
    ("leaps", "First leap and low swimming", "GJFFFOOD.BMP", [5, 6, 8, 15, 17, *range(19, 26)]),
    ("reactions", "Reactions and turns", "GJFFFOOD.BMP", [*range(26, 34), 44, 45, 48]),
    ("dives", "Diving back into the water", "GJFFFOOD.BMP", [34, 35, 36]),
    ("shark", "Second encounter: jaws, reactions and dive", "SHARK.BMP", list(range(25, 38))),
]
...
def main():
    ...
    rows, nav, sections = [], [], []
    for group, title, resource, frames in GROUPS:
    ...
    assert len(rows) == 39 and len({r["key"] for r in rows}) == 39
    ...
    record = {"schema_version": 1, "status": "awaiting_39_drawing_appearance_review", "generated_drawing_count": len(rows),
              ...
              "helpers": [pin(HERE / n) for n in ("build_review.py", "record_output.py", "review-template.html", "review.js")],
              ...
              "deferred_work": ["Original canvas placement, registration and water layering", "Native sequence and motion checks", "Bulk smoke and regression tests"]}

"""Build the 60-drawing worker and mermaid gallery with visible originals."""
import hashlib
import html
import json
...
def main():
    ...
    assert len(rows) == 60 and len({r["key"] for r in rows}) == 60
    ...
    record = {"schema_version": 1, "status": "awaiting_60_drawing_appearance_review", "generated_drawing_count": len(rows),
              ...
              "helpers": [pin(HERE / n) for n in ("build_review.py", "record_output.py", "review-template.html", "review.js", "gallery-groups.json", "prepare_references.py", "source-notes.md", "selection.json", "worker-tool-notes.md", "worker-rope-notes.md", "mermaid-source-notes.md", "tail-source-notes.md", "tail-005-006-notes.md")],
              "applied_authoring_lessons": {"path": "docs/art-style-learnings-pose-and-action.md", "role": "Reusable authoring notes; exact applied pose instructions are preserved in each generation request."},

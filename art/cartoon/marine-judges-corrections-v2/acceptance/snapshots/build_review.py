"""Build four user-directed corrections with original/earlier/corrected panels."""
import hashlib
import json
import os
...
for item in feedback["frames"]:
    f = item["frame"]
    version = versions[f]
    folder = HERE / "generation/GJDIVE.BMP"
    ...
    sections.append(f'<section><h2 id="frame-{f}">GJDIVE {f}</h2><p>{notes[f]}</p><div class="row">' + "".join(cards) + "</div></section>")
    rows.append(row)
...
review = {"schema_version": 1, "status": "awaiting_four_marine_judge_corrections_review", "frames": rows,
...
assert len(rows) == 4
print("Built four corrections with twelve comparison panels")

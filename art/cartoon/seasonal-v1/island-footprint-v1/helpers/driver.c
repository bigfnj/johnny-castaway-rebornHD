/* Observation-only driver. Engine rendering and archive inputs are unchanged. */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include "ads.h"
                         ...
                         int x, int y, uint16 frame, uint16 image)
{
    ...
    if (image < MAX_BMP_SLOTS && slot->bmpNames[image] &&
        !strcmp(slot->bmpNames[image], "HOLIDAY.BMP")) {
...
{
    if (argc != 7 || !artStyleSelect(argv[5])) return 2;
    ...
    soundInit();
    adsInit();
    islandState.lowTide = atoi(argv[6]);
    ...
    /* Real same-heading native wait at A gives standing/context and finite return. */
    adsPlayWalk(0, 0, 0, 0);
    adsReleaseIsland();
    soundEnd();
    graphicsEnd();

...
source = root / 'build/seasonal-v1/native-candidate-v4'
target = root / 'art/cartoon/seasonal-v1/native/evidence-candidate-v4'
...
for frame, member in enumerate(expected):
    path = root / f'art/cartoon/seasonal-v1/candidates/v4/BMP/HOLIDAY.BMP/{frame:03}.png'
...
verify_runtime(summary['selected_archive']['holiday_members_sha256'])
archive = root / 'build/seasonal-v1/cartoon-seasonal-draft-v4.zip'
...
retain(source / 'v3-delta.json', 'records/v3-delta.json')
...
retain(root / 'art/cartoon/seasonal-v1/candidates/v4/export-report.json', 'records/export-report.json')
retain(root / 'build/seasonal-v1/check_v4_delta.py', 'helpers/check_v4_delta.py')
retain(Path(__file__), 'preserve_candidate.py')
for name in ['recipe-v4.json', 'export-verification-v4-smoke.json',
             'export-verification-v4-regression.json', 'pumpkin-appearance-v4.json']:
    retain(root / 'art/cartoon/seasonal-v1' / name, 'records/' + name)
...
binding = {'status': 'PASS', 'selected_runtime': selected,
           ...
           'scope': 'All four actual native-selected members equal the four candidates/v4 exported PNG bytes. Technical source identity, not human approval.'}
...
appearance = json.loads((target / 'records/pumpkin-appearance-v4.json').read_bytes())
assert appearance['appearance_approved'] is True
assert appearance['raw_sha256'] == json.loads((target / 'records/recipe-v4.json').read_bytes())['frames'][0]['source_sha256']
previous = target.parent / 'evidence-candidate-v3/evidence.json'
report = {'schema_version': 1, 'status': 'PASS',
          'human_art_approval': 'Pumpkin decay appearance approved in linked record; composed scene/shoreline compatibility and production remain unapproved.',
          'files_sha256': copied,
          'linked_previous_candidate': {'path': previous.relative_to(root).as_posix(), 'sha256': sha(previous)},

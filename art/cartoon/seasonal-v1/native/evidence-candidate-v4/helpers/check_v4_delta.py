"""Bounded readback: the decay revision changes only pumpkin package/native pixels."""
...
current = base / 'native-candidate-v4'
old = base / 'native-candidate-v3'
report_path = current / 'v3-delta.json'
...
before = members(base / 'cartoon-seasonal-draft-v3.zip')
after = members(base / 'cartoon-seasonal-draft-v4.zip')
...
report = {'status': 'PASS', 'method': 'Read actual two private archives and30 matched native capture pairs; previous captures/images remain unchanged.',
          'old_archive_sha256': sha((base / 'cartoon-seasonal-draft-v3.zip').read_bytes()),
          'new_archive_sha256': sha((base / 'cartoon-seasonal-draft-v4.zip').read_bytes()),
          ...
          'scope': 'Other3 seasonal assets and all original production payloads are exact. Pumpkin changes are inside its unchanged full canvas; this does not assert every non-decay source pixel is unchanged or grant art approval.'}

...
source = root / 'build/seasonal-v1/native-candidate-v3'
target = root / 'art/cartoon/seasonal-v1/native/evidence-candidate-v3'
...
for frame, member in enumerate(expected):
    path = root / f'art/cartoon/seasonal-v1/candidates/v3/BMP/HOLIDAY.BMP/{frame:03}.png'
...
verify_runtime(summary['selected_archive']['holiday_members_sha256'])
archive = root / 'build/seasonal-v1/cartoon-seasonal-draft-v3.zip'
...
retain(source / 'v2-delta.json', 'records/v2-delta.json')
...
retain(root / 'art/cartoon/seasonal-v1/candidates/v3/export-report.json', 'records/export-report.json')
retain(root / 'build/seasonal-v1/check_v3_delta.py', 'helpers/check_v3_delta.py')
...
binding = {'status': 'PASS', 'selected_runtime': selected,
           ...
           'scope': 'All four actual native-selected members equal the four candidates/v3 exported PNG bytes. Technical source identity, not human approval.'}

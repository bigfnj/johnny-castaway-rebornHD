...
source = root / 'build/seasonal-v1/native-candidate-v2'
target = root / 'art/cartoon/seasonal-v1/native/evidence-candidate-v2'
...
for frame, member in enumerate(expected):
    path = root / f'art/cartoon/seasonal-v1/candidates/v2/BMP/HOLIDAY.BMP/{frame:03}.png'
...
verify_runtime(summary['selected_archive']['holiday_members_sha256'])
archive = root / 'build/seasonal-v1/cartoon-seasonal-draft-v2.zip'
...
retain(root / 'art/cartoon/seasonal-v1/candidates/v2/export-report.json', 'records/export-report.json')
...
binding = {'status': 'PASS', 'selected_runtime': selected,
           ...
           'scope': 'All four actual native-selected members equal the four candidates/v2 exported PNG bytes. Technical source identity, not human approval.'}

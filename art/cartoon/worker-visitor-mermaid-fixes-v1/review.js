...
async function render() {
  ...
  await Promise.all(data.assets.flatMap(row => (row.blocked ? ["original"] : ["cartoon", "original"]).map(async kind => {
  ...
  const blocked = data.assets.filter(row => row.blocked).length;
  status.textContent = `${data.assets.length - blocked} drawings ready; ${blocked} blocked original shown.`;
}

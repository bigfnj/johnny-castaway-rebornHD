...
This art-only batch contains the independent lightbulb, exclamation mark and question mark from `LITEBULB.BMP` frames 000, 001 and 004. Johnny drawings 002 and 003 are excluded. Human appearance approval is pending.
...
`generation/001-generated-v2.png` is a preserved alternative for composited review. A targeted haze-removal edit had already started when alpha sampling showed that several broad amber areas in v1 were hidden RGB at alpha 0. Both versions retain hidden amber RGB in those sampled areas. No visible-halo defect or successful fix has been established; judge the actual transparent composition before choosing a version. The first result remains the default draft, with neither version approved.

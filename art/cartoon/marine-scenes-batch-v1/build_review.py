"""Build the 36-drawing marine scene appearance gallery, with originals kept visible."""
import hashlib
import html
import json
...
ROOT = HERE.parents[2]
GROUPS = [
    ("octopus", "Octopus: reaching and turning", "GJCATCH2.BMP", [0, 3, *range(25, 36)]),
    ("catch", "Octopus carrying fish", "GJCATCH2.BMP", [36, 37, 38, 39]),
    ("starfish", "Diving judges: starfish", "GJDIVE.BMP", [16, 17, 18]),
    ("crab", "Diving judges: crab", "GJDIVE.BMP", [19, 20, 21]),
    ("fish", "Diving judges: fish", "GJDIVE.BMP", [24, 25, 26]),
    ("gull", "Diving judges: gull", "GJDIVE.BMP", [27, 28, 29]),
    ("water", "Splash and ripple effects", "SPLASH.BMP", list(range(7))),
]
...
def main():
    ...
    assert len(rows) == 36 and len({r["key"] for r in rows}) == 36
    ...
    record = {"schema_version": 1, "status": "awaiting_36_drawing_appearance_review", "generated_drawing_count": len(rows),

"""Prepare exact marine-scene source drawings and enlarged inspection copies."""
import hashlib
import io
import json
import zipfile
...
GROUPS = {'GJCATCH2.BMP': [0, 3, *range(25, 40)],
          'GJDIVE.BMP': [*range(16, 22), *range(24, 30)],
          'SPLASH.BMP': list(range(7))}
...
def main():
    ...
    source = {'schema_version':1, 'count':36, 'scope':'Exact source drawings for a new appearance review',
              'palette_limit':index['palette_limit'], **provenance, 'contacts':contacts, 'assets':rows}
    save(HERE / 'reference/source.json', source)
    plan = {'schema_version':1, 'target_new_drawings':36, 'user_batch_default':[36,48],
            'resources':GROUPS, 'source_record':pin(HERE/'reference/source.json'),
            'groups':{'octopus':'17 eight-arm-menace poses, including rear views and fish held by tentacles',
                      'judges':'12 starfish, crab, fish and gull drawings holding blank diving scorecards',
                      'water':'7 distinct splash and settling-ripple drawings'},
            'excluded':'GJDIVE shadow planes 022/023 and numeric overlays 031-039; no Johnny fragments selected.',
            'art_direction':'Original silhouette, eye visibility, overlap and prop placement govern each pose. Shared generated keys control materials only. Cards remain blank for separate score overlays.',
    ...
    print('Prepared 36 exact source drawings and inspection copies.')

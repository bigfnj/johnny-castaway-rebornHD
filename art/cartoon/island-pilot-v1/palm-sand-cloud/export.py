#!/usr/bin/env python3
"""Reproduce the four static candidate PNGs in a new output directory."""
import argparse
import hashlib
import io
import json
...
REPO = HERE.parents[3]
EXPECTED_PATHS = [f'BMP/BACKGRND.BMP/{n:03}.png' for n in (0, 12, 13, 15)]
...
def main():
    ...
    if ([a['path'] for a in recipe['assets']] != EXPECTED_PATHS or
        ...
        parser.error('recipe.json: expected exactly the four recorded static asset paths')
    ...
    print(json.dumps({'status': 'all four recorded candidate PNG hashes matched',

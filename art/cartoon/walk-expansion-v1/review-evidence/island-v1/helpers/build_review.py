...
<title>Johnny's new walk on the island</title><style>
...
</style><main><h1>Johnny's new walk on the island</h1><p>Current HD walk on the left, Cartoon candidate on the right. Watch the steps and the arrival.</p>
...
<p id="status">Loading native captures...</p><div class="panels"><section class="panel"><h2>Current HD walk</h2><canvas id="baseline" width="1280" height="960"></canvas></section><section class="panel"><h2>Cartoon candidate</h2><canvas id="candidate" width="1280" height="960"></canvas></section></div>
<p>The final standing pose still uses the existing HD artwork in both views.</p><details><summary>Technical details</summary><p class="muted">Both views use the current Cartoon island. The baseline uses the packaged HD walking sprites, not original-executable captures or the decoded supplied-original sprites. These are paired native Linux captures from the same saved executable and the same unmodified B-to-A route. The candidate archive adds six sprites; every prior archive member remains unchanged. The recorded timing includes background updates and the 1.6-second arrival hold. It does not establish original executable rendering or wall-clock speed.</p><p id="technical"></p><p class="muted">There are 47 observed displays, including a zero-duration completion witness. Repeating restarts the recorded route; it does not invent a return walk. Close-up uses the same native pixels with a matching camera rectangle in both views. No interpolated poses or image repainting.</p></details><p id="failure" role="alert"></p></main>
...
function draw(){if(!ready)return;const row=data.frames[index];for(const kind of ['baseline','candidate']){const c=$(kind),ctx=c.getContext('2d');ctx.imageSmoothingEnabled=false;if($('view').value==='walk'){c.width=400;c.height=280;ctx.imageSmoothingEnabled=false;ctx.drawImage(images[kind][index],row.draw_xy[0]*2-100,row.draw_xy[1]*2-40,400,280,0,0,400,280);}else{c.width=1280;c.height=960;ctx.drawImage(images[kind][index],0,0);}}$('status').textContent=row.frame===18?'Arrival pose':`Walking pose ${row.pose_index+1} / 23`;$('technical').textContent=`Native display ${index+1} / ${data.frames.length} · JOHNWALK.BMP ${String(row.frame).padStart(3,'0')} · ${row.logical_ms} ms · next interval ${row.duration_ms} ms · ${row.comparison}`;window.nativeReviewState={ready,index,frame:row.frame,position,playing,view:$('view').value,routeLength:data.frames.length};}
...
def main():
    ...
    baseline = HERE / 'baseline-v2/full'
    ...
    for a, b in zip(left['displays'], right['displays']):
        ...
        for kind, folder, record in [('baseline', baseline, a), ('candidate', candidate, b)]:
            ...
            from run_baseline import png_bytes

"""Measure the fixed018 v5 export and preserve its selected technical study."""
...
WORK = ROOT / 'build/standing018-proportions/stage-v5'
OUT = BUNDLE / 'export/trials-foot-v5'
...
def comparison(a, b, rows):
    ...
    return {
        ...
        'v5_alpha8_bounds_relative_rows': box(ba >= 8),
    }


raw_paths = [BUNDLE / '018-torso-v2.png', BUNDLE / '018-foot-v5.png']
raws = [rgba(path) for path in raw_paths]
runtime_paths = [ROOT / f'build/standing018-proportions/stage-v{v}/runtime/BMP/JOHNWALK.BMP/018.png' for v in (2, 5)]
...
result = {
    'schema_version': 1,
    'scope': 'Technical source measurement only. Separate v5 authoring smoke/regression passed; this record does not represent color correction, native capture or human acceptance.',
    ...
    'raw_corner_rgba': [v5[y, x].tolist() for x, y in [(0, 0), (1023, 0), (0, 1535), (1023, 1535)]],
    ...
    'target_observation': 'V5 far sole is144.6HD within the requested range. Near sole is146.1HD. This is a source-contour comparison, not proof of native sand contact.',
...
files = {}
for name in ['staging.json', 'preview/recipe.json', 'preview/export-report.json', 'runtime/recipe.json', 'runtime/export-report.json', 'tests/smoke.json', 'tests/regression.json']:
    files[name] = (WORK / name).read_bytes()
files['measurements.json'] = (json.dumps(result, indent=2) + '\n').encode()
files['measure_foot_v5.py'] = Path(__file__).read_bytes()
for name, payload in files.items():
    path = OUT / name
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_bytes(payload)
    assert path.read_bytes() == payload
record = {'schema_version': 1, 'scope': result['scope'], 'files_sha256': {name: sha(payload) for name, payload in files.items()}, 'measurement_source_path': Path(__file__).relative_to(ROOT).as_posix(), 'raw_sources_sha256': {p.relative_to(ROOT).as_posix(): sha(p.read_bytes()) for p in raw_paths}, 'runtime_pngs_retained_in_ignored_build': result['runtime'], 'commands': ['python -B art/cartoon/standing018-proportions-v1/export/stage.py --source art/cartoon/standing018-proportions-v1/018-foot-v5.png --source-sha256 97922bacf481925f29640606e305ba1747b7de5fb1b01439e4fcdaf8e2621c5e --work build/standing018-proportions/stage-v5', 'python -B build/standing018-proportions/stage-v5/authoring/export.py --prepare --source 018-foot-v5.png --output build/standing018-proportions/stage-v5/preview --preview-only', 'python -B build/standing018-proportions/stage-v5/authoring/export.py --recipe build/standing018-proportions/stage-v5/preview/recipe.json --output build/standing018-proportions/stage-v5/runtime', 'python -B build/standing018-proportions/measure_foot_v5.py'], 'command_results': ['PASS byte-exact018 authoring staging', 'PASS arrival018 PREVIEW ONLY', 'PASS arrival018 runtime-canvas candidate written; not promoted', 'PASS measurement and byte-exact evidence preservation'], 'interpreter': 'Python3.11.15 with Pillow12.3.0, NumPy2.4.6'}

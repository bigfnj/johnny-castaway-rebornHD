"""Measure the fixed018 v4 export and preserve this rejected-fit study only."""
...
WORK = ROOT / 'build/standing018-proportions/stage-v4'
OUT = BUNDLE / 'export/trials-foot-v4'
...
def comparison(a, b, rows):
    ...
    return {
        ...
        'v4_alpha8_bounds_relative_rows': box(ba >= 8),
    }


raw_paths = [BUNDLE / '018-torso-v2.png', BUNDLE / '018-foot-v4.png']
raws = [rgba(path) for path in raw_paths]
runtime_paths = [ROOT / f'build/standing018-proportions/stage-v{v}/runtime/BMP/JOHNWALK.BMP/018.png' for v in (2, 4)]
...
result = {
    ...
    'raw_corner_rgba': [v4[y, x].tolist() for x, y in [(0, 0), (1023, 0), (0, 1535), (1023, 1535)]],
    ...
    'target_observation': 'V4 far sole is143.1HD, still0.9 to1.9HD above the requested range. Near sole is146.1HD. This is a source-contour comparison, not proof of native sand contact.',
...
files['measure_foot_v4.py'] = Path(__file__).read_bytes()
for name, payload in files.items():
    path = OUT / name
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_bytes(payload)
    assert path.read_bytes() == payload
record = {'schema_version': 1, 'scope': result['scope'], 'files_sha256': {name: sha(payload) for name, payload in files.items()}, 'measurement_source_path': Path(__file__).relative_to(ROOT).as_posix(), 'raw_sources_sha256': {p.relative_to(ROOT).as_posix(): sha(p.read_bytes()) for p in raw_paths}, 'runtime_pngs_retained_in_ignored_build': result['runtime'], 'commands': ['python -B art/cartoon/standing018-proportions-v1/export/stage.py --source art/cartoon/standing018-proportions-v1/018-foot-v4.png --source-sha256 6ba5a7bdce36a0a8ed4e3912baa1415c3be4b6d38a437f289ae8a7d02dccb83a --work build/standing018-proportions/stage-v4', 'python -B build/standing018-proportions/stage-v4/authoring/export.py --prepare --source 018-foot-v4.png --output build/standing018-proportions/stage-v4/preview --preview-only', 'python -B build/standing018-proportions/stage-v4/authoring/export.py --recipe build/standing018-proportions/stage-v4/preview/recipe.json --output build/standing018-proportions/stage-v4/runtime', 'python -B build/standing018-proportions/measure_foot_v4.py'], 'command_results': ['PASS byte-exact018 authoring staging', 'PASS arrival018 PREVIEW ONLY', 'PASS arrival018 runtime-canvas candidate written; not promoted', 'PASS measurement and byte-exact evidence preservation'], 'interpreter': 'Python3.11.15 with Pillow12.3.0, NumPy2.4.6'}

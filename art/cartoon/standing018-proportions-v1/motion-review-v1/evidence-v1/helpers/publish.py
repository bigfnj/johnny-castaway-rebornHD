...
def publish(review, negatives_path, finish_existing=False):
    target = SERVER / SLUG
    assert target.exists() if finish_existing else not target.exists(), 'explicit new publication or finish existing018 review'
    ...
    for name, raw in files.items():
        path = target / name
        if finish_existing:
            assert path.read_bytes() == raw, 'finish exact prior published file:' + name
        else:
    ...
    if finish_existing:
        prior = build.load(review / 'publication-browser.json')
        assert prior['status'] == 'PASS' and prior['html_sha256'] == record['files_sha256']['review.html'] and prior['checker_sha256'] == build.sha(Path(check.__file__).read_bytes()), 'finish exact already-passed served browser check'
    else:
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            for clip in build.CLIPS:
                ...
                page.wait_for_function('window.nativeReviewState&&nativeReviewState.ready', timeout=90000, polling=50)
    ...
    result['finished_existing_after_screenshot_helper_timeout'] = finish_existing
...
if __name__ == '__main__':
    ...
    p.add_argument('--finish-existing', action='store_true')
    a = p.parse_args()
    publish(a.review, a.negatives, a.finish_existing)

"""Export only arrival 018 at the reviewed rear family's fixed scale.

Prepare a recipe with --prepare --source 018-pose-v1.png, or reproduce with
...
REFERENCE_HASHES = {
    "source.json": "bd2ba0025626593641c575b794d9380bf66613dae9b7f456d5715aa810d9ceae",
    "metadata.json": "130dcd25e05c45e22143adca37cb139679523f8dc4a84abce14efc9a170982d5",
    "018-native.png": "d4390dbd4e9df53d506dbedf540c4a2a0d867c23d3c5221743dad0bf07d2dbd8",
    "018-nearest8.png": "228783a6a48523333a069592318308967e75537b83bb1a446c85c69fa7181c7a",
}
...
def measure_cap(image):
    ...
    require(bounds is not None and bounds[1] < 300, "cap-outline:018")
    y = bounds[1]
...
def prepare(name):
    ...
    with Image.open(io.BytesIO(data)) as image:
        require(image.mode == "RGBA" and image.size == (1024, 1536), "prepare-canvas:018")
        cap = measure_cap(image)
    tx, ty = TARGET[0] - cap[0] * SCALE, TARGET[1] - cap[1] * SCALE
    return {
        "schema_version": 1,
        "scope": "Technical arrival-018 review only; no runtime or motion approval.",
        ...
        "target_basis": "Original018 row0 span x[6,11) has midpoint8.5, doubled to17HD; y0.25 is the inherited deliberate filter margin.",
...
def render(recipe, preview_only=False):
    original = reference()
    require(recipe["reference_hashes"] == REFERENCE_HASHES, "recipe-reference:018")
    require(recipe["schema_version"] == 1 and
            ...
            "render-contract:018")
    require([item["frame"] for item in recipe["frames"]] == [FRAME], "only-arrival:018")
    item = recipe["frames"][0]
    data = source_bytes(item["source"])
    require(sha(data) == item["source_sha256"], "source-identity:018")
    with Image.open(io.BytesIO(data)) as source:
        require(source.mode == "RGBA" and source.size == (1024, 1536) and
                item["generated_canvas"] == [1024, 1536], "source-canvas:018")
        ...
        require(item["cap_raw"] == cap and item["cap_target_hd"] == TARGET and
                ...
                all(math.isclose(a, b, rel_tol=0, abs_tol=1e-12)
                    for a, b in zip(item["affine_forward"], expected)), "fixed-registration:018")
        canvas = [n * 2 for n in original["canvas"]]
        require(item["runtime_canvas"] == canvas, "runtime-canvas:018")
        width, height = canvas
        ...
        require(preview_only or fits, "runtime-overhang:018")
        ...
        images = {"padded/018.png": png(padded)}
        if not preview_only:
            images["BMP/JOHNWALK.BMP/018.png"] = png(fixed)
    report = {
...
def main():
    ...
    parser.add_argument("--source", default="018-pose-v1.png", help="PNG basename in this authoring folder")
    ...
    try:
        ...
        return 1
    print("PASS arrival018; " + ("PREVIEW ONLY: runtime sprite not written" if args.preview_only

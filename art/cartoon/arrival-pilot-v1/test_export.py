"""Arrival018 authoring smoke, isolated behavioral checks and executed mutants.
...
def synthetic(path, top=40, size=(1024, 1536), overhang=False):
    ...
    if overhang:
        draw.rectangle((0, top + 100, 20, top + 110), fill=(40, 60, 80, 255))
    image.save(path)


GUARDS = {
    "reference-source": "reference-file:source.json",
    "reference-metadata": "reference-file:metadata.json",
    "reference-native": "reference-file:018-native.png",
    "reference-nearest8": "reference-file:018-nearest8.png",
    "source-path": "source-path:018",
    "prepare-canvas": "prepare-canvas:018",
    "cap-outline": "cap-outline:018",
    "recipe-reference": "recipe-reference:018",
    "render-contract": "render-contract:018",
    "only-arrival": "only-arrival:018",
    "source-identity": "source-identity:018",
    "source-canvas": "source-canvas:018",
    "fixed-registration": "fixed-registration:018",
    "runtime-canvas": "runtime-canvas:018",
    "runtime-overhang": "runtime-overhang:018",
...
def check_case(name, script, source_recipe, legacy_path, scratch):
    ...
    if name in POSITIVES:
        ...
        if name == "recipe-byte-identity":
            ...
            if name == "preview-overhang":
                assert set(images) == {"padded/018.png"}
                assert report["preview_only"] and not report["runtime_sprites_written"]
                assert not report["runtime_fit_all_source_centers"]
            else:
                assert set(images) == {"padded/018.png", "BMP/JOHNWALK.BMP/018.png"}
                assert report["runtime_fit_all_source_centers"]
                with Image.open(io.BytesIO(images["BMP/JOHNWALK.BMP/018.png"])) as image:
                    assert image.mode == "RGBA" and image.size == (64, 154)
                ...
                if name == "legacy-filter-parity":
                    ...
                    legacy.FRAMES = [18]
                    old_recipe = legacy.prepare({18: "fixture.png"})
    ...
    if name.startswith("reference-") and name != "recipe-reference":
        ...
        recipe["frames"][0]["runtime_canvas"] = [64, 156]
...
def main():
    ...
    parser.add_argument("--legacy", type=Path, default=HERE.parent / "walk-expansion-v1/export.py")
    ...
    if args.phase == "smoke":
        ...
        with Image.open(HERE / "reference/018-native.png") as native, Image.open(HERE / "reference/018-nearest8.png") as zoom:
            assert native.mode == zoom.mode == "RGBA"
            assert sha(native.tobytes()) == original["rgba_sha256"]
            assert zoom.size == (256, 616) and zoom.tobytes() == native.resize(zoom.size, Image.Resampling.NEAREST).tobytes()
        print("SMOKE PASS exact original018 reference and nearest8", flush=True)
        baseline = args.output / "smoke-export"
        r = cli(args.export, args.recipe, baseline)
        assert r.returncode == 0, r.stderr
        with Image.open(baseline / "BMP/JOHNWALK.BMP/018.png") as image:
            assert image.mode == "RGBA" and image.size == (64, 154) and image.getchannel("A").getbbox()
        assert len(list(baseline.rglob("*.png"))) == 2
        print("SMOKE PASS exact018 fixed-canvas export", flush=True)
        repeated = args.output / "smoke-repeated"
        r = cli(args.export, args.recipe, repeated)
        assert r.returncode == 0, r.stderr
        for relative in ("BMP/JOHNWALK.BMP/018.png", "padded/018.png"):

"""Build 7 original/previous/corrected worker comparisons."""
import hashlib
import html
import json
import os
...
ROOT = HERE.parents[2]
GROUPS = {
    'tools': 'Hammer grips and rope placement',
    'arms': 'Arm ownership',
    'body': 'Whole body facing',
}
...
def main():
    ...
    for item in feedback['frames']:
        ...
        row = {'key': key, 'resource': resource, 'frame': frame, 'group': item['group'],
               ...
               'original_binding': {k: v for k, v in item['original'].items() if k != 'url'},
               'prior_output_binding': {k: v for k, v in item['prior_raw'].items() if k != 'url'},
               'images': {}}
        cards = []
        for kind, title, path in [
            ('original', 'Original', ROOT / item['original']['path']),
            ('earlier', 'Previous Cartoon', ROOT / item['prior_raw']['path']),
    ...
    review = {
        'schema_version': 1, 'status': 'awaiting_7_worker_tool_pose_corrections_appearance_review',
        'scope': 'Only 7 worker tool/pose corrections. No implied approval of other drawings.',
        ...
        'helpers': [pin(HERE / p) for p in ('build_review.py', 'review.js', 'review-template.html',
                                           'record_output.py', 'prepare_feedback.py', 'arm-source-notes.md', 'hammer-source-notes.md', 'body-rope-source-notes.md')],

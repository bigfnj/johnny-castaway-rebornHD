...
$repo  = 'D:\.ai-work\worktrees\johnny-cartoon-clouds'
...
& python -B (Join-Path $repo 'tests\test_wave_renderer.py') --exe $exe --archive $sourceZip --phase smoke --work 'D:\.ai-work\worktrees\johnny-cartoon-clouds\build\seasonal-final-windows\clouds-feature-v1\wave-smoke'
if ($LASTEXITCODE -ne 0) {
...
& python -B (Join-Path $repo 'tests\test_palm_renderer.py') --driver $palmDriver --archive $sourceZip --phase smoke --work 'D:\.ai-work\worktrees\johnny-cartoon-clouds\build\seasonal-final-windows\clouds-feature-v1\palm-smoke'
if ($LASTEXITCODE -ne 0) {
...
if (-not $SmokeOnly) {
    ...
    & python -B (Join-Path $repo 'tests\test_wave_renderer.py') --exe $exe --archive $sourceZip --work 'D:\.ai-work\worktrees\johnny-cartoon-clouds\build\seasonal-final-windows\clouds-feature-v1\wave-regression'
    if ($LASTEXITCODE -ne 0) { $failed = $true }

    Write-Step 'regression (palm alpha, clipping and fallback)'
    & python -B (Join-Path $repo 'tests\test_palm_renderer.py') --driver $palmDriver --archive $sourceZip --work 'D:\.ai-work\worktrees\johnny-cartoon-clouds\build\seasonal-final-windows\clouds-feature-v1\palm-regression'
    if ($LASTEXITCODE -ne 0) { $failed = $true }

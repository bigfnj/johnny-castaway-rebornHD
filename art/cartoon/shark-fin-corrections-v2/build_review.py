"""Build the two anatomy corrections with original/earlier/corrected panels."""
import hashlib
import json
import os
...
notes = {"026": "The shape beneath the chin is a gray fin, following 027/028. The pink tongue and large open mouth are removed.",
         "035": "The body is mostly underwater, with the gray fin clearly exposed above the splash."}
rows, sections = [], []
for item in feedback["frames"]:
    f = item["frame"]
    version = versions[f]
    folder = HERE / "generation/GJFFFOOD.BMP"
    ...
    sections.append(f'<section><h2 id="frame-{f}">GJFFFOOD {f}</h2><p>{notes[f]}</p><div class="row">' + "".join(cards) + "</div></section>")
    rows.append(row)
...
review = {"schema_version": 1, "status": "awaiting_two_fin_corrections_review", "frames": rows,
...
print("Built two fin corrections with six comparison panels")

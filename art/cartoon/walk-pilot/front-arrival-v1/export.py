"""Export only arrival 017 at the reviewed walking family's fixed scale.

Prepare a recipe with --prepare --source 017-pose-v1.png, or reproduce with
...
REFERENCE_HASHES = {
    "preparation.json": "1bea660fb89ed83714cbb9b634d0e1a6e020950b92c4d908000d7010daa563c6",
    "017-original-native.png": "f8579f9122c1f2e585d960f8de94159bcd17de8e29148ed5e796f2819c81e92c",
    "017-original-nearest8.png": "d3ac095f60c8bbd4ff33250641fbb27f51f938f816fdbff8d82450d8add5f42a",
}
...
def reference():
    for name, expected in REFERENCE_HASHES.items():
        require(sha((HERE / "reference" / name).read_bytes()) == expected,
                "reference-file:" + name)
    return json.loads((HERE / "reference/preparation.json").read_bytes())["original_frame"]


def source_bytes(name):
    require(isinstance(name, str) and name not in ("", ".", "..") and
            "/" not in name and "\\" not in name and ":" not in name and
            Path(name).suffix.lower() == ".png", "source-path:017")
...
def measure_cap(image):
    ...
    require(bounds is not None and bounds[1] < 300, "cap-outline:017")
    y = bounds[1]
...
def prepare(name):
    ...
    with Image.open(io.BytesIO(data)) as image:
        require(image.mode == "RGBA" and image.size == (1024, 1536), "prepare-canvas:017")
        cap = measure_cap(image)
    tx, ty = TARGET[0] - cap[0] * SCALE, TARGET[1] - cap[1] * SCALE
    return {
        "schema_version": 1,
        "scope": "Technical arrival-017 review only; no runtime or motion approval.",
        ...
        "target_basis": "Original017 row0 span x[27,31) has midpoint29, doubled to58HD; y0.25 is the inherited deliberate filter margin.",
...
def render(recipe, preview_only=False):
    original = reference()
    require(recipe["reference_hashes"] == REFERENCE_HASHES, "recipe-reference:017")
    require(recipe["schema_version"] == 1 and
            ...
            "render-contract:017")
    require([item["frame"] for item in recipe["frames"]] == [FRAME], "only-arrival:017")
    item = recipe["frames"][0]
    data = source_bytes(item["source"])
    require(sha(data) == item["source_sha256"], "source-identity:017")
    with Image.open(io.BytesIO(data)) as source:
        require(source.mode == "RGBA" and source.size == (1024, 1536) and
                item["generated_canvas"] == [1024, 1536], "source-canvas:017")
        ...
        require(item["cap_raw"] == cap and item["cap_target_hd"] == TARGET and
                ...
                all(math.isclose(a, b, rel_tol=0, abs_tol=1e-12)
                    for a, b in zip(item["affine_forward"], expected)), "fixed-registration:017")
        canvas = [n * 2 for n in original["canvas"]]
        require(item["runtime_canvas"] == canvas, "runtime-canvas:017")
        width, height = canvas
        ...
        require(preview_only or fits, "runtime-overhang:017")
        ...
        images = {"padded/017.png": png(padded)}
        if not preview_only:
            images["BMP/JOHNWALK.BMP/017.png"] = png(fixed)
    report = {
...
def main():
    ...
    parser.add_argument("--source", default="017-pose-v1.png", help="PNG basename in this authoring folder")
    ...
    try:
        ...
        return 1
    print("PASS arrival017; " + ("PREVIEW ONLY: runtime sprite not written" if args.preview_only

"""Arrival017 authoring smoke, isolated behavioral checks and executed mutants.
...
def cli(script, recipe, output, preview=False):
...
GUARDS = {
    "reference-preparation": "reference-file:preparation.json",
    "reference-native": "reference-file:017-original-native.png",
    "reference-nearest8": "reference-file:017-original-nearest8.png",
    "source-path": "source-path:017",
    "prepare-canvas": "prepare-canvas:017",
    "cap-outline": "cap-outline:017",
    "recipe-reference": "recipe-reference:017",
    "render-contract": "render-contract:017",
    "only-arrival": "only-arrival:017",
    "source-identity": "source-identity:017",
    "source-canvas": "source-canvas:017",
    "fixed-registration": "fixed-registration:017",
    "runtime-canvas": "runtime-canvas:017",
    "runtime-overhang": "runtime-overhang:017",
...
def check_case(name, script, source_recipe, legacy_path, scratch):
    ...
    if name in POSITIVES:
        ...
        if name == "recipe-byte-identity":
            ...
            if name == "preview-overhang":
                assert set(images) == {"padded/017.png"}
                assert report["preview_only"] and not report["runtime_sprites_written"]
                assert not report["runtime_fit_all_source_centers"]
            else:
                assert set(images) == {"padded/017.png", "BMP/JOHNWALK.BMP/017.png"}
                assert report["runtime_fit_all_source_centers"]
                with Image.open(io.BytesIO(images["BMP/JOHNWALK.BMP/017.png"])) as image:
                    assert image.mode == "RGBA" and image.size == (80, 150)
                    assert image.getchannel("A").getextrema()[1] == 255
                if name == "legacy-filter-parity":
                    legacy = load(legacy_path, "frozen_rear_export")
                    legacy.HERE = authoring
                    # Reuse frozen 018 filter implementation with this fixture's
                    ...
                    expected = {name.replace("018", "017"): data for name, data in expected.items()}
    ...
    if name.startswith("reference-") and name != "recipe-reference":
        ...
        recipe["reference_hashes"]["preparation.json"] = "0" * 64
        ...
        recipe["frames"][0]["runtime_canvas"] = [80, 152]
...
def main():
    ...
    parser.add_argument("--legacy", type=Path, default=HERE.parents[1] / "arrival-pilot-v1/export.py")
    ...
    if args.phase == "smoke":
        ...
        with Image.open(HERE / "reference/017-original-native.png") as native, Image.open(HERE / "reference/017-original-nearest8.png") as zoom:
            assert native.mode == zoom.mode == "RGBA"
            assert sha(native.tobytes()) == original["rgba_sha256"]
            assert zoom.size == (320, 600) and zoom.tobytes() == native.resize(zoom.size, Image.Resampling.NEAREST).tobytes()
        print("SMOKE PASS exact original017 reference and nearest8", flush=True)
        baseline = args.output / "smoke-export"
        r = cli(args.export, args.recipe, baseline)
        assert r.returncode == 0, r.stderr
        with Image.open(baseline / "BMP/JOHNWALK.BMP/017.png") as image:
            assert image.mode == "RGBA" and image.size == (80, 150) and image.getchannel("A").getbbox()
        assert len(list(baseline.rglob("*.png"))) == 2
        print("SMOKE PASS exact017 fixed-canvas export", flush=True)
        repeated = args.output / "smoke-repeated"
        r = cli(args.export, args.recipe, repeated)
        assert r.returncode == 0, r.stderr
        for relative in ("BMP/JOHNWALK.BMP/017.png", "padded/017.png"):

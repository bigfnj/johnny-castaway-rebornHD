"""Shared 000/015 authoring smoke, behavioral checks and executed mutants.
...
def check_case(name, script, source_recipe, legacy_path, scratch, frame):
        ...
        if name == "original-cap-target":
            ...
            if name == "preview-overhang":
                ...
                with Image.open(io.BytesIO(images[f"BMP/JOHNWALK.BMP/{frame:03}.png"])) as image:
                    assert image.mode == "RGBA" and image.size == {0: (80, 152), 15: (80, 146)}[frame]
    ...
    if name.startswith("reference-") and name != "recipe-reference":
        ...
        recipe["reference_hashes"] = module.REFERENCE_HASHES[15 if frame == 0 else 0].copy()
    elif name == "render-contract":
        recipe["scale_exact"]["denominator"] = 11
    elif name == "recipe-frame":
        recipe["frames"][0]["frame"] = 15 if frame == 0 else 0
...
def main():
    ...
    parser.add_argument("--frame", type=int, choices=(0, 15), required=True)
    ...
    if args.phase == "smoke":
        ...
        with Image.open(baseline / f"BMP/JOHNWALK.BMP/{frame:03}.png") as image:
            assert image.mode == "RGBA" and image.size == {0: (80, 152), 15: (80, 146)}[frame] and image.getchannel("A").getbbox()
        ...
        if args.mutation_check:
            ...
            old = {0: "0: [54, .25]", 15: "15: [35, .25]"}[frame]
            assert code.count(old) == 1
            result, digest = isolated_case("original-cap-target", code.replace(old, {0: "0: [53, .25]", 15: "15: [34, .25]"}[frame]), args.recipe, args.legacy, args.output, frame)
            assert result.returncode == 1 and "FAILED (failures=1)" in result.stderr and "ERROR:" not in result.stderr

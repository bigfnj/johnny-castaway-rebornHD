"""Prepare an isolated future017 candidate; all existing ZIP members remain exact."""
import argparse
import copy
import hashlib
import json
...
ROOT = OUT.parents[2]
MEMBER = 'data/styles/cartoon/BMP/JOHNWALK.BMP/017.png'
HD_MEMBER = 'data/hd/BMP/JOHNWALK.BMP/017.png'
...
def main():
    ...
    require(sha(source.read_bytes()) == baseline['archive_sha256'], 'current baseline archive identity')
    raw = args.png.read_bytes()
    require(sha(raw) == args.expected_sha256, 'explicit candidate017 identity')
    ...
    with zipfile.ZipFile(source) as archive:
        ...
        require(MEMBER not in names, 'baseline has no Cartoon017')
        require(size == list(struct.unpack('>II', archive.read(HD_MEMBER)[16:24])), 'unchanged017 runtime canvas')
    ...
    with zipfile.ZipFile(target) as candidate:
        require(candidate.namelist() == names + [MEMBER], 'exactly one new Cartoon017 member')
        ...
        require(candidate.read(MEMBER) == raw, 'new017 exact bytes')
    (folder / '017.png').write_bytes(raw)
    report = {
        'scope': 'Private technical comparison only; no production edit or human approval. PNG fit/provenance remain authoring responsibilities.',
        ...
        'added_members': [{'frame': 17, 'member': MEMBER, 'new_sha256': sha(raw), 'canvas': size}],
    ...
    require(sha(source.read_bytes()) == baseline['archive_sha256'], 'production archive unchanged')

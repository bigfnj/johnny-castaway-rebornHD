"""Capture a future private017 candidate with the current-main native observer."""
import argparse
import json
import os
...
def compare(folder, phase, preparation, binding):
    ...
    report = capture.parse(destination, phase, capture.contract(), candidate017=True)
    for field in ('path_seed', 'api', 'actual_path', 'observed_rows', 'actual_draws',
                  'native_delay_returns_ticks', 'native_completed_waits', 'display_count', 'duration_ms'):
        capture.require(report[field] == expected[field], phase + ': exact baseline ' + field)
    dependencies = sorted('data/styles/cartoon/BMP/JOHNWALK.BMP/017.png' if name == 'data/hd/BMP/JOHNWALK.BMP/017.png' else name for name in expected['loaded_art'])
    ...
    for observed, prior in zip(report['displays'], expected['displays']):
        ...
        if frame in canvases:
            ...
            observed['comparison'] = 'different only inside placed017 canvas'
            changed += 1
        else:
    ...
    report.update({'control': False, 'changed_displays': changed, 'unchanged_displays': len(report['displays']) - changed,
                   ...
                   'command': command, 'scope': 'Private017 native candidate, same observer and route as current production28. Every travel display must remain identical; only the mirrored017 canvas may change.'})
    capture.save(destination / 'report.json', report)
    print(f'PASS candidate {phase}: {len(report["displays"])} displays; {changed} differ only inside placed017 canvas', flush=True)
    return report
...
def main():
    ...
    capture.save(folder / 'summary.json', {'status': 'PASS', 'phases': ['smoke', 'full'],
                 ...
                 'final_arrival': 'Cartoon017 mirrored at (293,243), only arrival canvas differs; all travel displays identical',

...
def main():
    ...
    with sync_playwright() as p:
        ...
        for width in (1280, 1600):
            ...
            for kind in ('baseline', 'candidate'):
                fit = page.locator('#' + kind).evaluate('c=>{const r=c.getBoundingClientRect();return r.left>=0&&r.right<=innerWidth&&r.top>=0&&r.bottom<=innerHeight&&Math.abs(r.width/c.width-r.height/c.height)<0.001;}')
        ...
        for label, old, new in [('timed-display', "position+=(time-lastTime)*Number($('speed').value)", 'position+=0'),
            ...
            if label == 'timed-display':
                ...
                arrival = next(i for i, row in enumerate(data['frames']) if row['frame'] == 17)

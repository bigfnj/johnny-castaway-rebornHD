...
void __wrap_platformUpdateWindow(PlatformWindow *window)
{
    ...
    if (!pixels || width != 1280 || height != 960 || pitch < width * 4)
        fatalError("FRONT OBSERVER: invalid capture surface");
    char name[64];
    snprintf(name, sizeof(name), "display-%03u.ppm", ++display_count);
    FILE *file = safe_fopen(name, "wb");
    if (fprintf(file, "P6\n%d %d\n255\n", width, height) < 0)
        fatalError("FRONT OBSERVER: capture header failed");
    ...
    for (int y = 0; y < height; ++y) {
        ...
        if (fwrite(row, sizeof(row), 1, file) != 1)
            fatalError("FRONT OBSERVER: capture pixels failed");
    }
    if (fclose(file)) fatalError("FRONT OBSERVER: capture close failed");
    printf("FRONT DISPLAY: %u logical_ms=%llu file=%s\n", display_count, logical_ms, name);
    fflush(stdout);
}
...
int main(int argc, char **argv)
{
    ...
    for (seed = 0; seed < 2000; ++seed) {
        srand(seed);
        int *path = calcPath(4, 0);
        if (path[0] == 4 && path[1] == 0 && path[2] == UNDEF_NODE) break;
    }
    if (seed == 2000) fatalError("FRONT DRIVER: no direct E,A,UNDEF path");
    debugMode = 1;
    srand(seed);
    printf("FRONT DRIVER: island_seed=11 path_seed=%u route=E,A,UNDEF api=adsPlayWalk(4,1,0,1) style=%s\n", seed, argv[1]);
    printf("FRONT ISLAND: highTide=%d offset=%d,%d raft=%d night=%d holiday=%d\n",
    ...
    graphicsEnd();
    zipvfs_shutdown();
    printf("FRONT DRIVER: finite route returned; cleanup complete; displays=%u logical_ms=%llu\n",

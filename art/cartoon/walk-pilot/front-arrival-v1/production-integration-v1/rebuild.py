"""Reproduce the approved four-standing-pose package from pinned inputs.
...
INPUTS_SHA = "5ee22544cb6a2b490b86addc2cfaa75d9085b1d60e4c3210bb6900c2739967c0"
...
def main():
    ...
    witness = "WITNESS standing-package " + sha(Path(__file__).read_bytes())
    ...
    require(len(before) == 2579, "baseline: expected2579 members")
    specs = load(HERE / inputs["runtime_sources"])["frames"]
    require([s["frame"] for s in specs] == [0, 15, 16, 17], "scope: expected four standing frames")
    expected = dict(before)
    additions = {PREFIX + s["path"]: s["sha256"] for s in specs}
    require(not set(additions).intersection(before), "scope: standing members must be additions")
    ...
    require(len(prior["assets"]) == 28 and len(ledger["assets"]) == 32, "ledger: expected28-to32 assets")
    require(ledger["assets"][:28] == prior["assets"], "ledger: previous28 rows changed")
    ...
    if args.promote:
        require(load(repo / "art/cartoon/pack.json") == ledger, "promotion: current ledger differs from frozen32-asset snapshot")
    ...
    print("SMOKE PASS four annotated recipes reproduce reviewed PNG and padded bytes", flush=True)
    ...
    require(smoke["assets"] == 32, "pack-smoke: expected32 accepted sprites/screens")
    print("SMOKE PASS art_pack validates32 approved PNGs", flush=True)
    ...
    require(len(actual) == 2583 and set(actual) - set(before) == set(additions), "built archive: wrong addition scope")
    ...
    damaged = PREFIX + "BMP/JOHNWALK.BMP/000.png"
    ...
    wrong.unlink()
    verify_protected(repo, inputs)
    print("REGRESSION PASS2583 members;2579 unchanged; exactly four additions; named000 corruption control fired", flush=True)
    report = {"schema_version": 1, "scope": "Four runtime reproductions, package smoke, complete member regression and executed corrupted-member control. Native/human evidence remains separate.",
              ...
              "archive_sha256": sha(built.read_bytes()), "member_count": len(actual), "cartoon_asset_count": 32,
              "unchanged_members": len(before), "added_members": additions, "removed_members": [], "changed_members": [],
              "prior28_pack_rows_unchanged": True, "runtime_manifest_unchanged": before[PREFIX + "manifest.json"] == actual[PREFIX + "manifest.json"],
    ...
    if args.promote:
        ...
        handle, name = tempfile.mkstemp(prefix=".standing-family-", suffix=".zip", dir=production.parent)
        os.close(handle)
        staged = Path(name)
        try:

...
def main():
    ...
    if args.candidate_version == 2:
        old = config.load_json(config.HERE.parent / 'candidate-selection.json')['frames'][1]
        assert old['frame'] == selection['frames'][1]['frame'] == 10
        changes = [('exporter', selection['exporter'], 'selected exporter binding:010'),
                   ('smoke', old['smoke'], 'selected smoke input binding:010'),
                   ('regression', old['regression'], 'selected regression input binding:010'),
                   ('checks', old['checks'], 'selected verification binding:010')]
        for field, replacement, label in changes:
            changed = copy.deepcopy(selection)
            changed['frames'][1][field] = copy.deepcopy(replacement)
            try:
                prepare_candidate.validate_selection(changed)
            except AssertionError as error:
                assert str(error) == label, 'wrong mixed-version failure:' + str(error)
                results.append({'mutation': '010 uses existing valid version1 ' + field + ' record', 'result': 'FIRED', 'expected_failure': label})
                print('FIRED ' + label, flush=True)
            else:
                raise AssertionError('mixed-version substitution SURVIVED:010:' + field)
        assert prepare_candidate.validate_selection(selection) == inputs, 'restored version2 input bindings'
    ...
    result_path.write_text(json.dumps({'status': 'PASS', 'candidate_version': args.candidate_version,
              ...
              'scope': 'Actual validator on the complete handoff: two same-canvas wrong-pose substitutions, plus four existing valid version1 tool/evidence substitutions for version2. Hashes establish identity; no human approval inferred.'}, indent=2) + '\n', encoding='utf-8')
    print(f'PASS restored handoff after {len(results)} exact named semantic-binding failures')

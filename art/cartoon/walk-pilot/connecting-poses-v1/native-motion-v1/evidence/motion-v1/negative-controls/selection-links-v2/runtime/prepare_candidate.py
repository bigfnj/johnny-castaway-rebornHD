...
def validate_selection(selection):
    ...
    for row in selection['frames']:
        ...
        exporter = row.get('exporter', selection['exporter'])
        test_tool = row.get('test_tool', selection['test_tool'])
        assert report['export_source_sha256'] == exporter['sha256'], 'selected exporter binding:' + label
        expected_tests = (exporter['sha256'], test_tool['sha256'], row['recipe']['sha256'], frame)
        for phase in ('smoke', 'regression'):
            proof = config.load_json(ROOT / row[phase]['path'])
            tested = proof['inputs_sha256']
            assert tuple(tested[key] for key in ('export', 'test', 'recipe', 'selected_frame')) == expected_tests and proof['phase'] == phase and proof['passed'] > 0, 'selected ' + phase + ' input binding:' + label
        checks = config.load_json(ROOT / row['checks']['path'])
        assert checks['selected_frame'] == frame and checks['source'] == row['source'] and checks['exporter'] == exporter and checks['tests'] == test_tool, 'selected verification binding:' + label
        ...
        assert True, 'selected runtime output binding:' + label

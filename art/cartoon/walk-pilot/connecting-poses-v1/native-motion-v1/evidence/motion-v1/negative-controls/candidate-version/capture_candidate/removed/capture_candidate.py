"""Capture the same native calls; changes are limited to placed connecting canvases."""
import argparse
import os
...
def compare_reports(report, expected, prep, folder, prior_folder):
    ...
    core.require(report['loaded_art'] == sorted(replacements.get(name, name) for name in expected['loaded_art']), 'only connecting dependencies change')
...
def compare(clip, phase, prep, binding):
    ...
    print(f'PASS candidate {clip} {phase}: {len(report["displays"])} displays; {changed} inside-connecting changes', flush=True)
    return report
...
def main():
    ...
    print('WITNESS connecting candidate SHA256=' + core.sha(Path(__file__).read_bytes()), flush=True)
    ...
    core.save(CANDIDATE / 'summary.json', {'status': 'PASS', 'candidate_version': args.candidate_version,
              'clips': results, 'archive_sha256': prep['archive_sha256'], 'baseline_sha256': prep['base_archive_sha256'],
              ...
              'scope': 'Each of six candidate clips smoke then full exact baseline comparison then fresh-process repeat. Three connecting canvases only; all prior-approved poses and backgrounds unchanged.'})
    print('PASS six candidate clips; exact timing and scoped pixels; production unchanged', flush=True)
...
if __name__ == '__main__':
    try:
        ...
        core.save(OUT / (CANDIDATE.name + '-failure.json'), {'traceback': traceback.format_exc(), 'helper_sha256': core.sha(Path(__file__).read_bytes())})
        raise

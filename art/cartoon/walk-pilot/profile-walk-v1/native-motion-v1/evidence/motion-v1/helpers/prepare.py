...
def main():
    ...
    with zipfile.ZipFile(archive) as packed:
        ...
        assert len(accepted) == 32, '32 accepted Cartoon PNGs retained'
        assert all(f'data/styles/cartoon/BMP/JOHNWALK.BMP/{i:03}.png' not in names for i in config.CANDIDATE_FRAMES), 'profile remains HD baseline'
...
'''
    ...
    record = {'source_commit': subprocess.check_output(['git', '-C', str(ROOT), 'rev-parse', 'HEAD'], text=True).strip(),
              ...
              'scope': 'Byte-exact current32-asset production baseline; four actual profile route clips; no candidate or production changes.'}

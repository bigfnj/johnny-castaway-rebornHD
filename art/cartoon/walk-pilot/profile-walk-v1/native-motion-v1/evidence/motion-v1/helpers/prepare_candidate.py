"""Build a private eight-pose package from an explicit technical export handoff."""
import argparse
import copy
import io
import json
...
def validate_selection(selection):
    assert [row['frame'] for row in selection['frames']] == list(config.CANDIDATE_FRAMES), 'exact eight profile candidates'
    inputs = {}
...
def main():
    ...
    with zipfile.ZipFile(base) as original, zipfile.ZipFile(package) as packed:
        assert packed.namelist() == names + list(additions), 'only eight named additions'
    ...
    record = {'status': 'PASS', 'scope': 'Private technical candidate only; no motion approval or production promotion.',
              ...
              'base_member_count': len(names), 'unchanged_prior_payloads': len(names), 'added_members': added,
    ...
    print(f'PASS private candidate adds eight poses; all{len(names)} prior members unchanged; SHA256={record["archive_sha256"]}')

"""Observe native profile routes, preserving all display events and timing."""
import json
import os
...
def parse(folder, phase, clip, candidate=False):
    ...
    require(f'TURN DRIVER: island_seed=11 path_seed=2 clip={clip} style=cartoon' in text, 'actual configured profile witness')
    ...
    for name, observed in segments.items():
        ...
        require(observed['chosen_path'] == reference['chosen_path'], 'actual selected direct path:' + name)
    ...
    if phase == 'smoke':
        ...
        require({d['actual_draw'][3] for d in displays if d['role'] == 'travel'} == set(config.CANDIDATE_FRAMES), 'all eight profile resources observed')
        require(any(d['role'] == 'ordinary turn' for d in displays) == config.CLIPS[clip]['ordinary'], 'configured ordinary003 context observed')
        require(displays[-1]['actual_draw'][3] == 0 and displays[-1]['role'] == 'arrival', 'actual approved standing000 arrival')
    ...
    for frame in (0, 15, 16, 17, 18, 24, 25, 26, 27, 28, 29):
    ...
    for frame in config.CANDIDATE_FRAMES:
        ...
        require(selected in loaded and absent not in loaded, 'selected profile dependency:' + str(frame))
    return {'status': 'PASS', 'clip': clip, 'phase': phase, 'segments': segments, 'displays': displays,
            ...
            'scope': 'Native port profile route, actual API priming/arrival and optional ordinary003 departure; all observed timestamps and origins. No original-binary timing claim.', 'candidate': candidate}
...
def main():
    print('WITNESS profile capture SHA256=' + sha(Path(__file__).read_bytes()), flush=True)
    ...
    save(BASE / 'summary.json', {'status': 'PASS', 'clips': reports, 'baseline_pack_sha256': binding['baseline_pack_sha256'], 'executable_sha256': binding['executable_sha256'],
         ...
         'verification': 'Each of four routes: smoke then full then exact fresh-process repeat. Independent compiled C trace checked before native rendering.',
         'scope': 'Current32-asset baseline, HD001-008; no candidates, publication or production changes.'})
    print('PASS four native profile clips and unchanged protected inputs', flush=True)

"""Profile 001-008 authoring smoke, behavioral checks and executed mutants.

Adapted from the frozen remaining-waits-v1 tests. Original references establish
all eight geometries; synthetic opaque/partial/transparent pixels test filtering.
...
CANVASES = {1: (96, 144), 2: (96, 146), 3: (80, 152), 4: (80, 148),
            5: (96, 144), 6: (96, 148), 7: (64, 150), 8: (80, 150)}
...
def check_case(name, script, source_recipe, legacy_path, scratch, frame):
    ...
    if name.startswith("reference-") and name != "recipe-reference":
        ...
        recipe["reference_hashes"] = module.REFERENCE_HASHES[1 if frame != 1 else 3].copy()
    elif name == "render-contract":
        recipe["scale_exact"]["denominator"] = 11
    elif name == "recipe-frame":
        recipe["frames"][0]["frame"] = 1 if frame != 1 else 3
        ...
        for invalid in (None, -1, 0, 9, 16, 17):
...
def main():
    ...
    parser.add_argument("--frame", type=int, choices=range(1, 9), required=True)
    ...
    if args.case:
        ...
        suite = unittest.defaultTestLoader.loadTestsFromTestCase(type("ProfileExport", (unittest.TestCase,),

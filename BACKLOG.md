...
| --- | --- |
| Expand Cartoon by complete motion families | Production now contains 12 walking poses and 15 island assets. The [production catalog](docs/knowledge-base/cartoon-production-catalog.md) tracks all 2,401 slots and links 51 supplied-original references. The rear cycle is [accepted in the native island review](art/cartoon/walk-expansion-v1/production-acceptance.json), including the discussed 8-to-9 transition. Next draw arrival/wait frame 018, which remains HD fallback, and review its transition with the already-approved turn frame 023. The [delivery record](docs/rear-cartoon-walk-verification.md) traces this pair's direction-table and story use. Then continue the [bounded plan](docs/cartoon-production-plan.md). Revisit the front walk afterward if its in-scene result warrants it. Other directions, low-tide foam, other ocean/cloud variants, holiday/raft scenery and most story animations use fallback. Keep "Cartoon (preview)" until coverage is complete. Palette-driven primitives and fades require visual review beyond PNG coverage. |
...
arrival 018 remains HD fallback.

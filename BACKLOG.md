...
| Restore Cartoon high-tide shoreline before more ground props | The [matched original comparison](art/cartoon/seasonal-v1/island-footprint-v1/README.md) proves that original wave003-011 sprites supply opaque sand beyond base000. Cartoon wave prompts removed that sand. In the matched high-tide phase, sampled ground near clovers retreats by up to13 HD pixels;6/15 original stem endpoints fall below Cartoon sand versus1/15 on the original. Base000's roughly2.5% opaque-area reduction is secondary at these endpoints. Restore the combined original sand/wave footprint at unchanged origins and validate all phases with clovers, pumpkin and tree. Then reassess the temporary v2 seasonal shrink/raise. Do not globally enlarge the island or attribute the earlier upper-shore018 contact problem to these lower waves. The diagnostic uses original artwork in the port, not original-executable color parity; low-tide layers retain HD fallback. |
| Review standalone seasonal drafts before resuming character production | HOLIDAY.BMP000-003 drafts (pumpkin, clover, decorated tree and New Year banner) are prepared in [seasonal-v1](art/cartoon/seasonal-v1/README.md), pending human scene approval and production integration. The pumpkin's red eyes, sharp evil grin and slight rot are approved; v4 preserves that appearance. Current ground-prop placement compensates for the confirmed shoreline defect above and must be rechecked after it is corrected. Follow with independent props, then vehicle directions and environmental animation. Some vehicle/story images contain Johnny or uncertain occupants; keep those with their character/scene groups. This ordering reduces anatomy review dependencies; no production speedup was measured. |

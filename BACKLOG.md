...
| --- | --- |
| Style the remaining sky and ocean states | The [cloud delivery](docs/cartoon-clouds-verification.md) completes ordinary moving clouds BACKGRND.BMP015-017 alongside the accepted high/low-tide compositions. The separate CLOUDS.BMP000-003 resource has no proved runtime use in the current source/script map; investigate reachability before authoring it. NIGHT.SCR and alternate ocean screens beyond OCEAN02 still use fallback. Review night/ocean states as coherent scenes, retaining the accepted clouds and both tide compositions. Keep Cartoon labeled as a partial preview. |

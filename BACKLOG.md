...
| --- | --- |
| Style the remaining environment states after seasonal integration | Native before/after checks for the selected offshore island show the existing fallback lower beach, rock and low-tide wave families in both versions. BACKGRND.BMP 001/002 and 030-041 remain unstyled; CLOUDS.BMP 000-003 and NIGHT.SCR also remain fallback. These are known coverage gaps, not newly introduced pixel differences outside the selected shoreline. Treat the low-tide ground and its waves as one scene group, preserving the approved high-tide island and offshore ripples. Review original source composition before generating replacements. Keep Cartoon labeled as a partial preview. |
...
| Finish Cartoon shoreline enlargement before more ground props | The user approved the [two-inlet sand shape](art/cartoon/shoreline-repair-v1/smooth-shore-v3/README.md) and chose [Cartoon: offshore ripples](art/cartoon/shoreline-repair-v1/integrated-shore-v1/wave-approaches-v1/selection.json) after the three-way clover animation comparison. Retain that displayed style; the incoming-wash alternative is an unselected reference. Named Cartoon canvas extents preserve logical positions and full-size props without shrinking or clipping. The selected version passed 24 native smoke captures, then 24 fresh-process exact repeats and six negative controls across seasonal props, low tide, shifted/night scenes and Johnny routes. After the [seasonal scene review](art/cartoon/shoreline-repair-v1/integrated-shore-v1/offshore-scene-review-v1/README.md), the user accepted the pumpkin and tree placements and requested visible attachments between the New Year banner and palm. That revision and the user's observation of more offshore waves in the prior preview need resolution before production integration. The [feedback record](art/cartoon/shoreline-repair-v1/integrated-shore-v1/seasonal-placement-review.json) preserves this scope. The earlier upper-shore 018 observation remains separate from this lower-shore correction. |
| Review standalone seasonal drafts before resuming character production | HOLIDAY.BMP000-003 drafts (pumpkin, clover, decorated tree and New Year banner) are prepared in [seasonal-v1](art/cartoon/seasonal-v1/README.md), pending human scene approval and production integration. The pumpkin's red eyes, sharp evil grin and slight rot are approved. V5 restores V1 prop size and registration with that approved pumpkin, replacing the reduced/raised placement that compensated for the shoreline defect. Review the full-size props against the enlarged island before promotion. Follow with independent props, then vehicle directions and environmental animation. Some vehicle/story images contain Johnny or uncertain occupants; keep those with their character/scene groups. This ordering reduces anatomy review dependencies; no production speedup was measured. |
...
| Check copied review evidence against the Git index before delivery | The fresh PR15 main checkout lacked three browser-execution records inside an ignored nested `build/` directory, although the feature worktree contained their correctly hashed bytes. Source/worktree and staged-file identity checks both passed because neither established that every declared copied file was staged. The final audit restored and explicitly staged all three unchanged records, then checked binder membership against the index. Add this completeness check to the maintained evidence tooling, with a dropped-file control, so later packs preserve every declared copied record. Keep intentional external original resources and scratch captures distinct from copied durable evidence. This was an evidence-storage omission; runtime art and all 2,594 package members were correct. |

...
| Add authoring metadata checks to routine CI | The current catalog/provenance checks are documented manual authoring checks and passed for this revision, but neither native gate nor CI invokes them. Add one Linux CI step that runs metadata smoke before regressions and the generator's reproduction check. The suite uses the standard library; avoid repeating it in every platform job or making runtime fixture builds depend on the full art history. This would catch stale generated catalogs or active approval pointers in later art changes. |

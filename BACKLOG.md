...
| Recover or account for missing sound assets | The archive has no sound11.wav or sound13.wav. Static disassembly finds one PLAY_SAMPLE 11 in GJCATCH2.TTM and no PLAY_SAMPLE 13 in the 41 shipped TTM files. This is a source occurrence count, not runtime scene coverage. Locate the original SCRANTIC data before substituting audio. |
| Broader malformed LZW validation | Short output is now rejected, but this is not a complete malformed-stream audit: getByte still substitutes zero at EOF and dictionary/code validation needs minimal fixtures plus original-corpus controls. Preserve the legitimate full-buffer return. |

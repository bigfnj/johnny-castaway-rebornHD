...
| Finish Cartoon shoreline enlargement before more ground props | The user clarified that the broader smooth coast must retain two original inward notches, around29% and67% across the island. [Two-inlet shoreline study](art/cartoon/shoreline-repair-v1/smooth-shore-v3/README.md) is the current draft, with original-source and previous-draft comparisons, pending human shape review. Its inherited uniform correction for generation zoom is display-only, not an approved runtime transform. The larger beach needs room beyond the original sprite-canvas union; do not shrink or clip it back into the old rectangles. After shape approval, define an explicit scenery-canvas extent in runtime and pack/catalog validation, preserve draw origins and full-size props, and fit the foam artwork to the new coast. Existing center foam can disappear behind the broader beach in one phase, so review every phase in motion. Then check low tide, shifted/night scenes and approved Johnny standing/walking contacts before production. [Earlier diagnosis](art/cartoon/seasonal-v1/island-footprint-v1/README.md) and [contact refinement](art/cartoon/shoreline-repair-v1/foam-contact-v2/README.md) retain the missing-sand and overlap findings. The lower-wave finding alone does not explain the earlier upper-shore018 issue. |

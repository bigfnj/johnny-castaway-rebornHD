...
int main(void)
{
    ...
    for (int j=0; j < 24; j++) {
        ...
        buffer = (char*)malloc((size_t)size * sizeof(char));
        fseek(f, offsets[j], SEEK_SET);
        fread(buffer, sizeof(char), (size_t)size, f);

        snprintf(filename, sizeof(filename), "../data/sound%d.wav", j + 1);
        fw = fopen(filename,"wb");
        fwrite(buffer, sizeof(char), (size_t)size, fw);
        fclose(fw);

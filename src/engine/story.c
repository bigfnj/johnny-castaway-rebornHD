                ...
                uint16 wantedFlags, uint16 unwantedFlags)
{
    ...
    if (numScenes == 0) {
        debugMsg("storyPickScene: no matching scenes found (wantedFlags=0x%x, unwantedFlags=0x%x)", wantedFlags, unwantedFlags);
        return NULL;
    }
...
void storyUpdateIslandFromDateAndTime(void)
{
    ...
    if (storyForcedHoliday >= 0) {
        islandState.holiday = storyForcedHoliday;
    }
    else {
...
void storyPlay(void)
{
    ...
    while (1) {
        ...
        if (finalScene == NULL) {
            debugMsg("storyPlay: no final scene found, restarting loop");
            continue;
        }
        ...
        if (!(finalScene->flags & FIRST)) {
            ...
            for (int i=0; i < 6 + (rand() % 14); i++) {
                ...
                if (scene == NULL) {
                    debugMsg("storyPlay: no scene found at index %d, skipping", i);
                    continue;
                }

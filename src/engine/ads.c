...
static void adsAddScene(uint16 ttmSlotNo, uint16 ttmTag, uint16 arg3)
{
    for (int i=0; i < MAX_TTM_THREADS; i++) {
        ...
        if (ttmThread->isRunning == TTM_RUNNING) {
    ...
    if (i >= MAX_TTM_THREADS) {
        debugMsg("(%d,%d) no free thread slot available - didn't add scene\n", ttmSlotNo, ttmTag);
        return;
    }
    ...
    if (ttmSlotNo >= MAX_TTM_SLOTS) {
        debugMsg("(%d,%d) ttmSlotNo out of range - didn't add scene\n", ttmSlotNo, ttmTag);
        return;
    }

    ttmThread->ttmSlot         = &ttmSlots[ttmSlotNo];
    ttmThread->isRunning       = TTM_RUNNING;
...
static int isSceneRunning(uint16 ttmSlotNo, uint16 ttmTag)
{
    for (int i=0; i < MAX_TTM_THREADS; i++) {
        ...
        if (    thread.isRunning == TTM_RUNNING
...
static void adsRandomAddScene(uint16 ttmSlotNo, uint16 ttmTag, uint16 numPlays,
                              uint16 weight)
{
    if (adsNumRandOps >= MAX_RANDOM_OPS)
        return;
...
static void adsRandomEnd(void)
{
    if (adsNumRandOps) {
       ...
       if (op == NULL) {
           debugMsg("RANDOM: no operation to choose from (all weights zero)");
           return;
       }
...
void adsPlaySingleTtm(const char *ttmName)  // TODO - tempo
{
    adsInit();
...
static void adsPlayTriggeredChunks(uint8 *data, uint32 dataSize, uint16 ttmSlotNo, uint16 ttmTag)
{
    ...
    if (numAdsChunksLocal) {
        ...
        for (int m=0; m < numMatches; m++) {
            adsPlayChunk(data, dataSize, adsChunksLocal[matchIdx[m]].offset);
        }

        for (int m=numMatches-1; m >= 0; m--) {
            for (int i=matchIdx[m]; i < numAdsChunksLocal-1; i++) {
                adsChunksLocal[i] = adsChunksLocal[i+1];
            }
            numAdsChunksLocal--;
        }
    }
...
void adsPlay(const char *adsName, uint16 adsTag)
{
    uint32 offset;
    uint8  *data;
    uint32 dataSize;
        ...
        for (int i=0; i < MAX_TTM_THREADS; i++) {
            ...
            if (ttmThreads[i].isRunning && ttmThreads[i].timer == 0) {
                ...
                if (ttmThreads[i].sceneTimer > 0) {
                    ...
                    if (ttmThreads[i].sceneTimer <= 0)
                        ttmThreads[i].isRunning = TTM_ENDING;
                }

                // Free terminated threads
                if (ttmThreads[i].isRunning == TTM_ENDING) {
                    ...
                    // Is there one (or more) IF_LASTPLAYED matching the terminated thread ?
                        ...
                        uint16 stoppedSlot = ttmThreads[i].sceneSlot;
                        uint16 stoppedTag  = ttmThreads[i].sceneTag;
                        adsStopScene(i);
                        if (!adsStopRequested)
                            adsPlayTriggeredChunks(data, dataSize, stoppedSlot, stoppedTag);
                    }
                }
            }
        }
...
void adsPlayBench(void)  // TODO - tempo
{
    ...
    for (int j=0; j < 3; j++) {
        ...
        for (int i=0; i < MAX_TTM_THREADS; i++)
            ttmThreads[i].isRunning = (i<numLayers ? TTM_RUNNING : TTM_FREE);
...
void adsInitIsland(void)
{
    ...
    ttmCloudsThread.isRunning = TTM_STATIC_LAYER;

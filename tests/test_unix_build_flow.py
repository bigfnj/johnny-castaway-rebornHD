...
def assert_case(mode, code, text, trace):
    full_trace = ['WITNESS build-executed', 'WITNESS png-smoke', 'WITNESS decoder-smoke', 'WITNESS frame-smoke', 'WITNESS platform-smoke']
    if LINUX: full_trace.append('WITNESS drawing-smoke')
    full_trace.extend(['WITNESS decoder-regression', 'WITNESS frame-regression', 'WITNESS platform-regression'])
...
def main():
    ...
    def verify(work):
        records=[]
        modes = ['clean','warning','build-fail','smoke-fail','decoder-smoke-fail','frame-smoke-fail','platform-smoke-fail','decoder-regression-fail','frame-regression-fail','platform-regression-fail']

...
def verify(page, case):
    ...
    if case == "retention":
        ...
        exact, clipped, error = state["log"][-3:]
...
def main():
    ...
    try:
        with sync_playwright() as playwright:
            ...
            cases = ["basic"] if options.phase == "smoke" else ["retention", "stderr", "length", "fatal"]
            if options.phase == "all": cases.insert(0, "basic")
            for case in cases:
                run(case, html)
                print(f"PASS index.html logging {case}", flush=True)
                records.append({"case": case, "result": "PASS"})
    ...
    if options.report:
        ...
        options.report.write_text(json.dumps({"html_sha256": hashlib.sha256(html).hexdigest(), "checks": records}, indent=2) + "\n", encoding="utf-8")
    return 0
